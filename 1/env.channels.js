...
(function envChannels(env) {
  ...
  Channel.prototype.close = function () {
    ...
    while (chan["[[Channel:next:length]]"] > 0) {
      ...
      if (next && !next.done) return next.resolve({value: undefined, done: true});
    }
  };
  ...
  Channel.prototype.send = function (v) {
    ...
    while (chan["[[Channel:next:length]]"] > 0) {
      ...
      if (next && !next.done) return next.resolve({value: v, done: false});
    }
    send = channelFifoPush(chan, "send", v);
    //if (chan["[[Channel:send:length]]"] <= chan["[[ChannelCapacity]]"]) send.resolve();
    if (chan["[[Channel:send:length]]"] <= chan["[[ChannelCapacity]]"]) return send.resolve();
    return send.promise;
  };
  Channel.prototype.next = function () {
    ...
    while (chan["[[Channel:send:length]]"] > 0) {
      ...
      if (send && !send.done) {
        send.resolve();
        //return env.Promise.resolve({value: send.value, done: false});
        return {value: send.value, done: false};
      }
    }
  ...
  // XXX does not work correctly due to possible random native Promise resolution
