...
(function envHttp(env) {
  ...
  env.parseHeaderRawParams = function (text) {
    ...
    var a = partitionToObject(";" + text, headerParamSearchRegExp), l = a.length, i, result = [];
    for (i = 0; i < l; i += 1) {
      if (i % 2) {
  ...
  env.parseRawMetadataList = function (text) {
    ...
    // API stability level: 1 - Experimental
    var a = partitionToObject("," + text, metadataSearchRegExp), l = a.length, i, m, result = [], c = null;
    for (i = 0; i < l; i += 1) {
      if (i % 2) {
        m = a[i];
